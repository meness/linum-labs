    ...
    <div className="grid grid-flow-row gap-4 md:grid-cols-3">
      {isLoadingListing &&
        new Array(6).fill(1).map(() => {
          return (
            <Skeleton className="h-[300px] rounded-[var(--nextui-radius-large)] transition-transform hover:scale-[1.02] hover:opacity-100 active:scale-100 active:opacity-100" />
          );
        })}
      {!isLoadingListing &&
